...
B<List Context>:
  ...
  my @albums = app::album->search( name => 'Legend' );
...
Advanced searching takes 1 or 2 parameters:
  ...
  app::album->search_where( { <args> }, [<order_by and limits>] );

Examples:

  my @albums = app::album->search_where({
    artist_id => { IN => [ 1, 2, 3 ] }
  });
...
Sometimes you just need to know how many records match your query:
  ...
  my $count = app::album->count_search( name => 'Bob Marley' );

Using C<count_where> you can make more interesting queries:

  my $count = app::album->count_search_where({
    name  => { LIKE => 'Legen%' },
    artist_id => { IN => [ 1, 2, 3 ] }
  });
...
objects of a pre-defined type:
  ...
  my $sth = app::artist->db_Main->prepare("SELECT * FROM artists WHERE name LIKE ?");
