...
B<List Context>:
  ...
  my @albums = My::Album->search( name => 'Legend' );
...
Advanced searching takes 1 or 2 parameters:
  ...
  My::Album->search_where( { <args> }, [<order-by and limits>] );

Examples:

  my @albums = My::Album->search_where({
    artist_id => { IN => [ 1, 2, 3 ] }
  });
...
Sometimes you just need to know how many records match your query:
  ...
  my $count = My::Album->count_search( name => 'Bob Marley' );

Using C<count_where> you can make more interesting queries:

  my $count = My::Album->count_search_where({
    name  => { LIKE => 'Legen%' },
    artist_id => { IN => [ 1, 2, 3 ] }
  });
...
objects of a pre-defined type:
  ...
  my $sth = My::Artist->db_Main->prepare("SELECT * FROM artists WHERE name LIKE ?");
...
it uses L<Ima::DBI> under the hood, you get all of its benefits.
